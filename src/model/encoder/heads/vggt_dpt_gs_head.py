...
class VGGT_DPT_GS_Head(DPTHead):
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], depths, imgs, patch_start_idx: int = 5, image_size=None, conf=None, frames_chunk_size: int = 8):
        ...
        print("I am in VGGT_DPT_GS_Head forward")
    ...
    def _forward_impl(self, encoder_tokens: List[torch.Tensor], imgs, patch_start_idx: int = 5, frames_start_idx: int = None, frames_end_idx: int = None):
        ...
        print("VGGT_DPT_GS_Head output shape:", out.shape)
        return out

class VGGT_DPT_Marble_Head(DPTHead):
    """
    DPT Head for marble (spherical) Gaussians.
    Outputs: opacity (1) + scale (1) + features (N)
    No rotation parameters since spheres are rotation-invariant.
    """
    def __init__(self, 
            dim_in: int,
            patch_size: int = 14,
            feature_dim: int = 256,  # Color/appearance features
            activation: str = "inv_log",
            conf_activation: str = "expp1",
            features: int = 256,
            out_channels: List[int] = [256, 512, 1024, 1024],
        ...
        # Calculate output_dim: 1 (opacity) + 1 (scale) + feature_dim (appearance)
        output_dim = 1 + 1 + feature_dim
        ...
        # Adjust head architecture
        head_features_1 = 128
        head_features_2 = 128 if feature_dim > 50 else 32
    ...
    def forward(
        ...
        # Process frames in chunks if needed
        if frames_chunk_size is None or frames_chunk_size >= S:
            return self._forward_impl(encoder_tokens, imgs, patch_start_idx)
        ...
        for frames_start_idx in range(0, S, frames_chunk_size):
            ...
            all_preds.append(chunk_output)
        print("shape of all_preds:", [p.shape for p in all_preds])
        return torch.cat(all_preds, dim=1)
    ...
    def _forward_impl(
        ...
        out = self.scratch.output_conv2(out)
        out = out.view(B, S, *out.shape[1:])
        print("VGGT_DPT_Marble_Head output shape:", out.shape)
        exit(0)
        return out
